...
1. c_budget (original)
2. c_budget_2d_array
3. c_budget_ragged_array
4. c_budget_array_of_structures
...
The problem is that the update and delete operations do not work on Windows. It has something to do with writing the new information to the temp file and removing the original budget.txt file. The program correctly writes the updated information to the temp_budget.txt file but can't remove the original file and rename the temporary file for some reason on Windows.
